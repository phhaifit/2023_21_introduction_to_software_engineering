...
import { createWorkspaceManagementRouter } from "./modules/workspace-management/interface/http/workspace-routes.ts";
import {
  createWorkspaceServerRuntime,
  type WorkspaceServerRuntime
} from "./modules/workspace-management/infrastructure/workspace-server-runtime.ts";
...
export type LocalAgentManagementRuntime = {
  ...
  workspaceManagement: WorkspaceServerRuntime;
};
...
export async function createLocalAgentManagementRuntime(): Promise<LocalAgentManagementRuntime> {
  ...
  const workspaceManagement = createWorkspaceServerRuntime();

  if (repository instanceof InMemoryAgentRepository) {
    await seedDemoAgents(repository);
  }

  startWorkspaceWorkerLoop(workspaceManagement);
  ...
  app.use(
    "/api/workspaces",
    createWorkspaceManagementRouter(workspaceManagement.httpDependencies)
  );
...
function startWorkspaceWorkerLoop(composition: WorkspaceServerRuntime): void {
  ...
  const tick = async () => {
    ...
    try {
      await composition.processOperation.processOne({
      ...
      console.error("[WorkspaceWorker] processOne error:", err);
    }

    try {
      await composition.publishOutbox.publishOne({
      ...
      console.error("[WorkspaceWorker] publishOne error:", err);
    }
  };
