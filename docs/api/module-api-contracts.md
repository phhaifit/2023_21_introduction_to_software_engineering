...
| `DELETE` | `/api/workspaces/:workspaceId/knowledge/documents/:documentId` | Workspace editor/admin | Route `workspaceId`; document lookup is workspace-scoped | Route parameters only | `ApiResponse` deletion acknowledgement | `implemented` | Deletes the local KB/RAG document copy, chunks, vector/index rows, ingestion/runtime jobs, and active agent grants. Google Drive source deletion does not delete the remote Drive file or account connection. |
