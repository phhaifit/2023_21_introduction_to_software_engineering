...
| Method | Path | Auth | Workspace Scope | Request Contract | Response Contract | Status | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `GET` | `/api/workspaces/:workspaceId/knowledge/documents` | Workspace member | Route `workspaceId` | Request context plus optional pagination | `ApiPaginatedSuccess` document summaries | `planned` | Lists indexed and pending documents. |
| `POST` | `/api/workspaces/:workspaceId/knowledge/documents` | Workspace editor/admin | Route `workspaceId` | document upload metadata and file boundary | `ApiResponse` document summary | `planned` | Creates metadata and delegates ingestion through worker boundary. |
| `DELETE` | `/api/workspaces/:workspaceId/knowledge/documents/:documentId` | Workspace editor/admin | Route `workspaceId` | Request context | `ApiResponse` deletion acknowledgement | `planned` | Removes document metadata and vector references through owned boundaries. |
| `GET` | `/api/workspaces/:workspaceId/knowledge/documents/:documentId/ingestion` | Workspace member | Route `workspaceId` | Request context | `ApiResponse` ingestion status | `planned` | Reports pending, indexed, or failed ingestion state. |
| `POST` | `/api/workspaces/:workspaceId/knowledge/retrieval` | Workspace member or runtime adapter | Route `workspaceId` | retrieval query DTO | `ApiResponse` retrieval result | `planned` | Uses vector adapter boundary and applies knowledge access rules. |
| `POST` | `/api/workspaces/:workspaceId/knowledge/access-assignments` | Workspace editor/admin | Route `workspaceId` | agent-document access DTO | `ApiResponse` access assignment summary | `planned` | Grants agent access to a document or collection. |
