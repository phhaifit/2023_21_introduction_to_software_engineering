...
| `POST` | `/api/workspaces/:workspaceId/agents` | `agents:manage` | Route `workspaceId` | `CreateAgentRequest` name, role, model, instructions, optional requested tool/knowledge intent | `ApiResponse` agent public summary | `implemented` | Existing route derives workspace from request context; requested tool/knowledge intent is validated but does not create assignments or grants. |
